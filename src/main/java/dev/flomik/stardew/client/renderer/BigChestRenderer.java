...
import dev.flomik.stardew.common.module.machinery.block.BlockBigChest;
import dev.flomik.stardew.common.module.machinery.blockentity.BlockEntityBigChest;
...
public class BigChestRenderer <T extends BlockEntityBigChest> implements BlockEntityRenderer<T> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(StardewMod.MODID, "big_chest"), "main");

    private static final ResourceLocation[] TEXTURES = new ResourceLocation[21];
    static {
        for (int i = 0; i < 21; i++) {
            TEXTURES[i] = new ResourceLocation(StardewMod.MODID, "textures/block/craftables/big_chest/big_chest_" + i + ".png");
        }
    }
    ...
    public BigChestRenderer(BlockEntityRendererProvider.Context context) {
        this(context, LAYER_LOCATION);
    }

    public BigChestRenderer(BlockEntityRendererProvider.Context context, ModelLayerLocation layer) {
    ...
    @Override
    public void render(BlockEntityBigChest entity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        BlockState state = entity.getBlockState();
        if (!(state.getBlock() instanceof BlockBigChest)) return;

        poseStack.pushPose();

        poseStack.translate(0.5D, 0.5D, 0.5D);
        Direction dir = state.getValue(BlockBigChest.FACING);
        ...
        int variant = state.getValue(BlockBigChest.VARIANT);
