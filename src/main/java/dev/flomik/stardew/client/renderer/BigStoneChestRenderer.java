...
import dev.flomik.stardew.common.module.craftables.block.BlockBigStoneChest;
...
import dev.flomik.stardew.common.module.craftables.blockentity.BlockEntityStoneChest;
...
public class BigStoneChestRenderer<T extends BlockEntityBigStoneChest> extends BigChestRenderer<T>{
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(new ResourceLocation(StardewMod.MODID, "big_stone_chest"), "main");

    private static final ResourceLocation[] TEXTURES = new ResourceLocation[21];
    static {
        for (int i = 0; i < 21; i++) {
            TEXTURES[i] = new ResourceLocation(StardewMod.MODID, "textures/block/craftables/big_stone_chest/big_stone_chest_" + i + ".png");
        }
    }

    public BigStoneChestRenderer(BlockEntityRendererProvider.Context context) {
    ...
    public static LayerDefinition createBodyLayer() {
        ...
        PartDefinition lid = partdefinition.addOrReplaceChild("lid", CubeListBuilder.create()
                        .texOffs(0, 24).addBox(1.0F, 0, -14.0F, 14.0F, 4.0F, 14.0F),
    ...
    @Override
    public void render(BlockEntityBigStoneChest entity, float partialTick, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        BlockState state = entity.getBlockState();
        if (!(state.getBlock() instanceof BlockBigStoneChest)) return;

        poseStack.pushPose();

        poseStack.translate(0.5D, 0.5D, 0.5D);
        Direction dir = state.getValue(BlockBigStoneChest.FACING);
